...
def build_commands(commands):
    commands.add("firewall-setup", firewall_setup, help="Setup firewall base config.")


def firewall_cmd(args, output = True):
    '''
    Execute the iptables shell command.

    '''
    x("/usr/bin/firewall-cmd " + args, output=output)

def configure_zones():
	firewall_cmd("--set-default-zone=dmz --permanent")

	#TODO: Find back net IF and set zone to internal
	if config.general.is_back_enabled():
		firewall_cmd("--zone=internal --change-interface=%s --permanent" % BACK_IF)	
