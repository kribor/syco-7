...
def restart_services():
    ...
    x("systemctl restart rpcbind")
    x("systemctl restart nfs-server")
    x("systemctl restart nfs-lock")
    x("systemctl restart nfs-idmap")


def stop_services():
    x("systemctl stop nfs-idmap")
    x("systemctl stop nfs-lock")
    x("systemctl stop nfs-server")
    x("systemctl stop rpcbind")


def enable_services_autostart():
    x("systemctl enable rpcbind")
    x("systemctl enable nfs-server")
    x("systemctl enable nfs-lock")
    x("systemctl enable nfs-idmap")


def disable_services_autostart():
    x("systemctl disable rpcbind")
    x("systemctl disable nfs-server")
    x("systemctl disable nfs-lock")
    x("systemctl disable nfs-idmap")
